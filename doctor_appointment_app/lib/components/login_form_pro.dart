...
class LoginFormPro extends StatefulWidget {
  final VoidCallback? onLoginSuccess; // Optional callback for login success

  const LoginFormPro({Key? key, this.onLoginSuccess}) : super(key: key);

  @override
  State<LoginFormPro> createState() => _LoginFormProState();
}

class _LoginFormProState extends State<LoginFormPro> {
                ...
                onPressed: () async {
                  if (_formKey.currentState!.validate()) {
                    ...
                    if (success) {
                      ...
                      // Check if the email is verified for pro users
                      if (user != null) {
                        bool isProVerified = await auth.checkProEmailVerified(user.email!);
                        if (!isProVerified) {
                          await auth.signOut(); // Sign out the unverified user

                          setState(() {
                            _errorMessage =
                            'Votre compte pro n\'est pas vérifié. Veuillez attendre l\'activation.';
                          });
                      ...
                      // Pro email is verified; proceed with the navigation
                      if (widget.onLoginSuccess != null) {
                        widget.onLoginSuccess!(); // Trigger callback if provided
                      } else {
                        MyApp.navigatorKey.currentState!
                            .pushReplacementNamed('/home_pro'); // Default navigation for pro users
                      }
                    } else {
                      setState(() {
                        _errorMessage =
